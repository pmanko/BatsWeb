...
function GetServerData(arg, context) {
    ...
    if (actionFlag == 'od') {
        ...
        $('#MainContent_ddPlayerTeam').val($('#MainContent_ddLineTeam').val())
    }
    else if (actionFlag == 'ot') {
        $('#MainContent_ddPlayerTeam').val($('#MainContent_ddOppTeam').val())
    }
...
function oneClickDateSelectionSuccess(dateRange) {
    ...
    $('.nav-tabs a[href=#dates]').tab('show');
...
$(document).on('change', "#MainContent_ddLineTeam", function (event) {
    makeServerRequest('lt', $(this).val())
});

$(document).on('change', "#MainContent_ddOppTeam", function (event) {
    makeServerRequest('ot', $(this).val())
});
...
function playerSelectedSuccess(player) {
    ...
    var parsed = parseInt(info[0]);
    if (parsed == 1 || parsed == 2 || parsed == 3 || parsed == 4 || parsed == 5) {
        $("#MainContent_lblLI" + parsed).val(info[1]);
    }
    else if (parsed == 6 || parsed == 7 || parsed == 8 || parsed == 9 || parsed == 10) {
        $("#MainContent_lblLE" + (parsed - 5)).val(info[1]);
    }
    else if (parsed == 11 || parsed == 12 || parsed == 13 || parsed == 14 || parsed == 15) {
        $("#MainContent_lblOI" + (parsed - 10)).val(info[1]);
    }
    else if (parsed == 16 || parsed == 17 || parsed == 18 || parsed == 19 || parsed == 20) {
        $("#MainContent_lblOE" + (parsed - 15)).val(info[1]);
    }
...
function LI(num) {
    makeServerRequest("li", num);
    $("#playerSelectionModal").modal('show');
}

function LE(num) {
    makeServerRequest("le", num);
    $("#playerSelectionModal").modal('show');
}

function OI(num) {
    makeServerRequest("oi", num);
    $("#playerSelectionModal").modal('show');
}

function OE(num) {
    makeServerRequest("oe", num);
    $("#playerSelectionModal").modal('show');
}

function LIC(num) {
    makeServerRequest("lic", num);
    $("#MainContent_lblLI" + num).val('');
}

function LEC(num) {
    makeServerRequest("lec", num);
    $("#MainContent_lblLE" + num).val('');
}

function OIC(num) {
    makeServerRequest("oic", num);
    $("#MainContent_lblOI" + num).val('');
}

function OEC(num) {
    makeServerRequest("oec", num);
    $("#MainContent_lblOE" + num).val('');
}
...
$(document).on("click", "#btnTeamSL", function (event) {
    ...
    //$("#rinkImagebtn").attr("src", "lineBreakdownRink.aspx?" + d.getTime());
    //$("#slModal").modal('show');
});
