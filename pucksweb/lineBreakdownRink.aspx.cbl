       class-id pucksweb.lineBreakdownRink is partial 
       ...
       procedure division using by value param-sender as object
           ...
           COMPUTE WS-HORIZ-ST ROUNDED = (PK320-RINK-X / 4)
           COMPUTE WS-VERT-ST ROUNDED = (PK320-RINK-Y / 4)
           ...
           MOVE 1 TO AA.
           if PK320-SHOT-CALL = "T" 
                PERFORM SHOW-T-SHOTS 1500 TIMES
           else     
           if PK320-SHOT-CALL = "O" 
      ...
      *        COMPUTE WS-Y2 = DOWNLOCY - UPLOCY.
       ...
       SHOW-T-SHOTS.
           IF PK320-T-SHOT-LOC-X(AA) = 0 AND PK320-T-SHOT-LOC-Y(AA) = 0
               NEXT SENTENCE
               ELSE
               COMPUTE WS-HORIZ ROUNDED = (PK320-T-SHOT-LOC-X(AA) / 2.67) * dimx
      *                    + WS-HORIZ-ST
               COMPUTE WS-VERT ROUNDED = (PK320-T-SHOT-LOC-Y(AA) / 2.67) * dimy
      *                    + WS-VERT-ST
               SUBTRACT 3 FROM WS-HORIZ
               SUBTRACT 3 FROM WS-VERT
               IF PK320-T-GOAL-FLAG(AA) = "Y"
       ...
       SHOW-O-SHOTS.
           IF PK320-O-SHOT-LOC-X(AA) = 0 AND PK320-O-SHOT-LOC-Y(AA) = 0
               NEXT SENTENCE
               ELSE
               COMPUTE WS-HORIZ = (PK320-O-SHOT-LOC-X(AA) / 2.67) * dimx
      *                    + WS-HORIZ-ST
               COMPUTE WS-VERT = (PK320-O-SHOT-LOC-Y(AA) / 2.67) * dimy   
      *                    + WS-VERT-ST
               SUBTRACT 3 FROM WS-HORIZ
               SUBTRACT 3 FROM WS-VERT
               IF PK320-O-GOAL-FLAG(AA) = "Y"
                   invoke g::DrawEllipse(type Pens::Black, WS-HORIZ, WS-VERT, 7, 7)
                   invoke g::FillEllipse(type SolidBrush::New(type Color::Chartreuse), WS-HORIZ, WS-VERT, 7, 7)
               ELSE
               IF PK320-O-GOAL-FLAG(AA) = "S"
                   invoke g::DrawEllipse(type Pens::RoyalBlue, WS-HORIZ, WS-VERT, 7, 7)
                   invoke g::FillEllipse(type SolidBrush::New(type Color::RoyalBlue), WS-HORIZ, WS-VERT, 7, 7)
               ELSE
                   invoke g::DrawEllipse(type Pens::Black, WS-HORIZ, WS-VERT, 7, 7)
