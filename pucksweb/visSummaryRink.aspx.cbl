       class-id pucksweb.visSummaryRink is partial 
       ...
       procedure division using by value param-sender as object
           ...
           MOVE 1 TO AA.
           PERFORM SHOW-V-SHOTS 500 TIMES.
      *    COMPUTE WS-HORIZ-ST ROUNDED = (PK360-RINK2-X / 4)
      *    COMPUTE WS-VERT-ST ROUNDED = (PK360-RINK2-Y / 4)
      *    MOVE 1 TO AA.
      ...
      *        invoke gHome::DrawRectangle(mypen, ws-x, ws-y, ws-x2, ws-y2).
       ...
       SHOW-V-SHOTS.
           IF PK360-V-SHOT-LOC-X(AA) = 0 AND PK360-V-SHOT-LOC-Y(AA) = 0
               NEXT SENTENCE
               ELSE
               COMPUTE WS-HORIZ ROUNDED = (PK360-V-SHOT-LOC-X(AA) / 2.67) * dimx
      *                   + WS-HORIZ-ST
               COMPUTE WS-VERT ROUNDED = (PK360-V-SHOT-LOC-Y(AA) / 2.67) * dimy
      *                   + WS-VERT-ST
              SUBTRACT 3 FROM WS-HORIZ
              SUBTRACT 3 FROM WS-VERT
               IF PK360-V-GOAL-FLAG(AA) = "Y"
